...
CREATE TABLE `movie` (
  ...
  `movie_summary` varchar(300) NOT NULL,
  `movie_pic` varchar(40) NOT NULL,
  `movie_type` int(11) NOT NULL,
  PRIMARY KEY (`movie_id`),
  KEY `movie_type` (`movie_type`),
  CONSTRAINT `movie_fk` FOREIGN KEY (`movie_type`) REFERENCES `type` (`type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `movie_comment` (
  ...
  CONSTRAINT `movie_comment_fk1` FOREIGN KEY (`movie_id`) REFERENCES `movie` (`movie_id`),
  CONSTRAINT `movie_comment_fk` FOREIGN KEY (`user_id`) REFERENCES `user` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
